...
def plot_feature_importance_demo():
    ...
    ax1.bar(range(len(importances)), importances[indices], color="steelblue", alpha=0.8)
    ...
    alphas = ard.alpha_
    ax2.bar(range(len(alphas)), alphas[indices], color="coral", alpha=0.8)
...
def plot_scalability_comparison():
    ...
    for n_samples, n_features in sizes:
        ...
        # Time ARD
        ard = ARDClassifier(learning_rate=0.05, max_iter=50, verbose=0, random_state=42)
        start = time.time()
    ...
    bars1 = ax1.bar(
        x - width / 2, ard_times, width, label="ARD", color="steelblue", alpha=0.8
    )
    bars2 = ax1.bar(
        x + width / 2,
        lr_times,
    ...
    # Plot 2: Accuracy
    ax2.plot(
        x, ard_scores, "o-", linewidth=2, markersize=8, label="ARD", color="steelblue"
    )
    ax2.plot(
        x,
        lr_scores,
...
def plot_sparsity_comparison():
    ...
    X, y = make_classification(
        n_samples=200, n_features=50, n_informative=10, n_redundant=5, random_state=42
    )
    X = StandardScaler().fit_transform(X)
    ...
    ax1.text(
        0.02,
        0.98,
        f"{n_important}/{len(ard.coef_)} features selected",
    ...
    colors_l1 = ["orange" if abs(c) > 1e-6 else "lightgray" for c in lr_l1.coef_[0]]

    ax2.bar(range(len(lr_l1.coef_[0])), lr_l1.coef_[0], color=colors_l1, alpha=0.8)
    ...
    ax2.text(
        0.02,
        0.98,
        f"{n_nonzero}/{len(lr_l1.coef_[0])} non-zero coefficients",
